...
Each of your assignments will begin with the following steps. Each lab will
start with a "Getting started" section but details will be a bit more sparse
than this. You can always refer back to this lab for a detailed list of the
steps involved for getting started with an assignment.

-   In our GitHub Organization, click 'Team' tab do find your team. Click
	'Memebers' tab lacated under green button 'New Team' and from drop-down menu
    select 'My teams'. Choose the team from the list.
-   Click on the GitHub repository (in short: 'repo') link located under
   "Repository" tab. Click the 'Fork' button to copy the repo to your account.

-   On GitHub, click on the green **Code** button, select **Use HTTPS** (this
    might already be selected by default, and if it is, you'll see the text
    **Clone with HTTPS** as in the image below). Click on the clipboard icon to
    copy the repo URL.

-   Go to RStudio and create a **New Project from Git Repo**. You will need to
    click on **New Project** --> **Version Control** --> **Git** --> and paste
    repo url in 'Ropository URL' field.
